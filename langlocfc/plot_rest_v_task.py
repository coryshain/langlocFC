        ...
        for eval_type in EVAL_TYPES:
            ...
            for a, atlas in enumerate(atlases):
                ...
                for k, key in enumerate(('FC',)):
                    ...
                    #     print(f'{atlas} {key} {e}: {len(_y_all[i])}')
                    _y = _df.mean(axis=0)
            ...
            plt.tight_layout()
            print(f'plots/performance_{parcellation_name}_{parcellation_type[-4:]}'
                        f'_{atlas_type}_{eval_type}{SUFFIX}')
            plt.savefig(f'plots/performance_{parcellation_name}_{parcellation_type[-4:]}'
                        f'_{atlas_type}_{eval_type}{SUFFIX}', dpi=DPI)
            plt.close('all')
            source.to_csv(f'plots/performance_{parcellation_name}_{parcellation_type[-4:]}'
                        f'_{atlas_type}_{eval_type}.csv', index=False)
