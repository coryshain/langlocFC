...
```bash
python -m parcellate.bin.plot <CONFIG_PATHS> -t atlas -A LANG LANA -e Lang_S-N
```
...
The analysis of laterality can be generated by `laterality.py`:

```bash
python -m langlocfc.laterality
```
...
Analyses of LangFC stability within and between subjects can be generated by `stability.py`:
...
and statistically tested by `test_pdd.py`:

```bash
python -m langlocfc.test_pdd
```

Plots of the supplementary analysis of template matching can be generated by `plot_template_matching.py`:

```bash
python -m langlocfc.plot_template_matching
```

Plots of the supplementary analysis of rest vs task parcellations can be generated by `plot_rest_v_task.py`:

```bash
python -m langlocfc.plot_rest_v_task
```
